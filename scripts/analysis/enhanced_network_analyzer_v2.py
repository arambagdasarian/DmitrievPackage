...
class EnhancedNetworkAnalyzerV2:
    def __init__(self):
        # Network parameters optimized for scraping-based dataset
        self.max_nodes_per_period = 2000  # Increased for comprehensive analysis
        self.min_edge_weight = 1  # Lower threshold for co-occurrence networks
        self.max_edges_per_period = 1500  # Increased for better coverage
        ...
        self.period_labels = {
            ...
            'post_crimea': 'Post-Crimea (2014-2017)', 
            'covid': 'COVID Period (2020-2022)',
            'war': 'War Period (2022-2025)'
        }
        
        # Enhanced community colors
        ...
        # Russian jurisdiction definition
            ...
            for _, row in group.iterrows():
                ...
                if entity not in entity_details:
                    entity_details[entity] = {
                        ...
                        'article_count': 0  # Will be updated
                    }
    ...
    def load_final_nodes_data(self):
        """Load and process data with enhanced filtering for scraping-based dataset"""
        ...
        try:
            ...
            # Count occurrences per entity (from scraping data)
    ...
    def process_periods_from_dataframe(self, df):
        """Process network data with enhanced co-occurrence analysis"""
        all_nodes = []
        all_edges = []
        ...
        for period in self.period_dates.keys():
            ...
            edges_list, entity_details = self.calculate_co_occurrence_network(period_df)
            ...
            if len(G.nodes()) == 0:
                print(f"No nodes in graph for {period}")
                continue
            ...
            metrics = self.calculate_network_metrics(G, entity_details)
            ...
            for node in G.nodes():
                ...
                # Calculate importance using multiple metrics
                ...
                article_cov = metrics['article_coverage'].get(node, 0)
                co_occur_strength = metrics['co_occurrence_strength'].get(node, 0)
                ...
                size = max(15, min(50, 15 + importance * 200 + degree * 2))
                ...
                all_nodes.append({
                    ...
                    'article_coverage': article_cov,
                    'co_occurrence_strength': round(co_occur_strength, 2),
                    'size': int(size),
            ...
            for edge in filtered_edges:
                all_edges.append({
                    ...
                    'weight': edge['weight'],
                    'period': period,
                    'width': max(0.5, min(5, edge['weight'] / 4)),  // Thinner edges
    ...
    def create_sample_data(self, period):
        ...
        for i, entity in enumerate(sample_entities):
            ...
            nodes.append({
                ...
                'eigenvector': round(np.random.random(), 4),
                'importance': round(np.random.random(), 4),
                'article_coverage': np.random.randint(10, 50),
                'co_occurrence_strength': round(np.random.random() * 5, 2),
    ...
    def create_enhanced_html(self, nodes_data, edges_data):
        """Create optimized HTML for large networks with enhanced features"""
...
<html>
<head>
    <title>Enhanced Network Analyzer V2 - Scraping-Based Analysis</title>
    <meta charset="utf-8">
    <style>
        * {{
            margin: 0;
...
<body>
    <div class="container">
        <div class="sidebar">
            <div class="header">
                <h1>Enhanced Network Analyzer V2</h1>
                <div class="subtitle">Scraping-Based Analysis - Up to 2000 nodes</div>
            </div>
            
            <div class="section">
                <h3>Dataset Context</h3>
                <div class="dataset-info">
                    Data from scraping articles mentioning Kirill Dmitriev. 
                    Networks based on entity co-occurrences in same articles.
                    Article_ID and Occurrences are key metrics.
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="performance-info">
                    Network optimized for up to 2000 nodes. All edges start grey, click nodes to activate colors.
                    Enhanced metrics for co-occurrence networks.
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Max Nodes Per View</span>
                        <span id="node-limit-value">2000</span>
                    </div>
                    <input type="range" class="slider" id="node-limit-slider" 
                           min="100" max="2000" value="2000" 
            ...
            <div class="section">
                ...
                <div class="filter-grid">
                    <button class="btn" onclick="filterByWeight(1)">1+</button>
    ...
    <script>
            ...
            if (selectedNodeId) {{
                ...
                const originalEdges = edges.get().map(edge => {{
                    return {{
                        ...
                        opacity: 1,
                        width: Math.max(1, Math.min(10, edge.weight / 2))
                    }};
                }});
        ...
        function showNodeDetails(nodeId) {{
            ...
            nodeInfo.innerHTML = `
                ...
                <div><strong>Article Coverage:</strong> ${{nodeData.article_coverage}}</div>
                <div><strong>Co-occurrence Strength:</strong> ${{nodeData.co_occurrence_strength}}</div>
        ...
        function showAllPeriods() {{
            clearHighlight();
            const limitedData = getLimitedData(allNodesData, allEdgesData, Math.min(currentNodeLimit, 1000));
            updateNetworkData(limitedData.nodes, limitedData.edges);
        }}
        ...
        function exportData() {{
            const exportData = {{
                ...
                metadata: {{
                    ...
                    datasetContext: 'Scraping-based analysis of articles mentioning Kirill Dmitriev'
                }}
            }};
            ...
            a.href = url;
            a.download = 'enhanced_network_data_v2.json';
    ...
    def run_enhanced_analysis(self):
        """Run the enhanced analysis with scraping-based dataset context"""
        print("🚀 Starting Enhanced Network Analysis V2 - Scraping-Based Context")
        print("=" * 80)
        ...
        if all_nodes:
            ...
            article_coverage = [node['article_coverage'] for node in all_nodes]
            print(f"   Average article coverage: {np.mean(article_coverage):.1f}")
            print(f"   Max article coverage: {max(article_coverage)}")
        
        # Create HTML
        html_content = self.create_enhanced_html(all_nodes, all_edges)
        
        # Save HTML file
        filename = "enhanced_network_analyzer_v2.html"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        print("\n" + "=" * 80)
        print("✅ ENHANCED NETWORK ANALYZER V2 - SCRAPING-BASED ANALYSIS!")
        print("=" * 80)
...
if __name__ == "__main__":
    analyzer = EnhancedNetworkAnalyzerV2()
    result_file = analyzer.run_enhanced_analysis()
    
    print(f"\n🎯 ENHANCEMENTS FOR SCRAPING-BASED DATASET:")
    print("=" * 60)
    print("✅ 1. Co-occurrence Network Construction:")
    print("   - Properly handles Article_ID grouping")
    print("   - Calculates entity co-occurrences in same articles")
    print("   - Edge weights represent co-occurrence frequency")
    
    print("\n✅ 2. Enhanced Network Metrics:")
    print("   - Article coverage: how many articles mention each entity")
    print("   - Co-occurrence strength: average weight of connections")
    print("   - Eigenvector centrality for influence measurement")
    print("   - Multi-metric importance calculation")
    
    print("\n✅ 3. Dataset Context Awareness:")
    print("   - Understands scraping-based data structure")
    print("   - Proper handling of Article_ID and Occurrences")
    print("   - Context-appropriate filtering and analysis")
    
    print("\n✅ 4. Performance Optimizations:")
    print("   - Handles large datasets (852,866 rows) efficiently")
    print("   - Optimized for up to 2000 nodes per view")
    print("   - Smart physics management for large networks")
    
    print("\n✅ 5. Accurate Calculations:")
    print("   - Proper co-occurrence edge calculations")
    print("   - Enhanced centrality measures for scraping data")
    print("   - Community detection optimized for co-occurrence networks")
    
    print("\n✅ 6. User Interface Enhancements:")
    print("   - Dataset context information displayed")
    print("   - Enhanced node details with article coverage")
    print("   - Improved filtering for co-occurrence networks")
