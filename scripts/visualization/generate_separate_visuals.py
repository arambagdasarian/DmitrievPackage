#!/usr/bin/env python3
"""
Generate Separate Visualizations - One Graph Per File
Based on refined dataset: Dmitriev_Node_Sheet_092025_SH_AF_102025.xlsx

This script creates individual visualization files for each requested analysis.
"""
...
def create_pie_chart_per_period(period_data, attribute, period):
    """Create a single pie chart for one period and one attribute"""
    ...
    ax.set_title(f'{attr_name} Composition - {period}\n({len(df)} records, {df["Entity"].nunique()} entities)',
...
def create_evolution_graph(period_data, attribute):
    """Create evolution line graph for one attribute across periods"""
    ...
    for value in all_values:
        ...
        ax.plot(PERIODS, y_values, marker='o', linewidth=2.5, markersize=8, label=value)
    
    ax.set_xlabel('Period', fontsize=13, fontweight='bold')
    ax.set_ylabel('Percentage (%)', fontsize=13, fontweight='bold')
    attr_name = attribute.replace('/', ' / ')
    ax.set_title(f'{attr_name} Evolution Across Periods', fontsize=16, fontweight='bold', pad=15)
...
def create_top50_comparison(period_data, attribute, period):
    ...
    attr_name = attribute.replace('/', ' / ')
    ax1.set_title(f'Overall Network\n{attr_name}', fontsize=14, fontweight='bold')
    for autotext in autotexts1:
        autotext.set_color('white')
        autotext.set_fontweight('bold')
    
    # Top 50 pie chart
    colors2 = sns.color_palette("husl", len(top50_counts))
    ...
    ax2.set_title(f'Top 50 Actors\n{attr_name}', fontsize=14, fontweight='bold')
    for autotext in autotexts2:
        autotext.set_color('white')
        autotext.set_fontweight('bold')
    
    fig.suptitle(f'{period} - {attr_name} Comparison', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def create_sector_comparison_chart(period_data, period):
    ...
    bars1 = ax.bar(x - width, overall_pct, width, label='Overall Network', color='#3498db', alpha=0.8)
    bars2 = ax.bar(x, top50_pct, width, label='Top 50', color='#e74c3c', alpha=0.8)
    bars3 = ax.bar(x + width, top20_pct, width, label='Top 20', color='#f39c12', alpha=0.8)
    
    ax.set_xlabel('Sector', fontsize=13, fontweight='bold')
    ax.set_ylabel('Percentage (%)', fontsize=13, fontweight='bold')
    ax.set_title(f'Sector Composition Comparison - {period}\nOverall vs Top 50 vs Top 20',
...
# ============================================================================
# ANALYSIS 4: TRENDS ACROSS PERIODS (PRIVATE TO STATE SHIFT)
# ============================================================================

def create_clustering_trend_chart(period_data, attribute):
    """Create stacked area chart showing clustering trends"""
    # Calculate percentages for each period
    period_data_list = []
    for period in PERIODS:
        df = period_data[period]
    ...
    df_pivot = df_plot.pivot(index='Period', columns='Attribute', values='Percentage').fillna(0)
    df_pivot = df_pivot.reindex(PERIODS)
    ...
    ax.stackplot(PERIODS, *[df_pivot[col] for col in df_pivot.columns],
                labels=df_pivot.columns, colors=colors, alpha=0.7)
    
    attr_name = attribute.replace('/', ' / ')
    ax.set_xlabel('Period', fontsize=13, fontweight='bold')
    ax.set_ylabel('Percentage (%)', fontsize=13, fontweight='bold')
    ax.set_title(f'{attr_name} Clustering Trends Across Periods', fontsize=16, fontweight='bold', pad=15)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10)
    ax.grid(True, alpha=0.3, axis='y')
    ax.set_ylim(0, 100)
    ...
    # Save
    filename = f'{OUTPUT_DIR}/4_clustering_trend_{attribute.replace("/", "_")}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✓ Saved: {filename}")
...
def create_louvain_community_visualization(period_data, period, attribute):
    """Create Louvain community network colored by attribute"""
    df = period_data[period]
    ...
    attr_values = [G.nodes[node].get(attribute.replace('/', '_'), 'Unknown') for node in G.nodes()]
    unique_attrs = sorted(set(attr_values))
    
    # Create color mapping
    colors_palette = sns.color_palette("husl", len(unique_attrs))
    ...
    # Draw nodes
    node_sizes = [G.nodes[node].get('Occurrences', 1) * 10 for node in G.nodes()]
    nx.draw_networkx_nodes(G, pos, ax=ax, node_color=node_colors, 
                          node_size=node_sizes, alpha=0.8, edgecolors='black', linewidths=0.5)
    
    # Draw labels for top nodes
    top_nodes = sorted(G.nodes(), key=lambda x: G.nodes[x].get('Occurrences', 0), reverse=True)[:20]
    labels = {node: node[:30] for node in top_nodes}
    nx.draw_networkx_labels(G, pos, labels, ax=ax, font_size=8, font_weight='bold')
    
    # Create legend
    legend_elements = [plt.Line2D([0], [0], marker='o', color='w', 
                                  markerfacecolor=color_map[attr], markersize=10, label=attr)
    ...
    ax.set_title(f'Louvain Communities - {period}\nColored by {attr_name}\n'
                f'({G.number_of_nodes()} nodes, {G.number_of_edges()} edges, {len(set(partition.values()))} communities)',
...
def create_attribute_cluster_visualization(period_data, period, attribute):
    """Create network visualization colored by attribute to show attribute-based clusters"""
    df = period_data[period]
    G = build_network_from_period_data(df, min_edge_weight=2)
    ...
    # Draw edges - color by whether they're internal or external
    edge_colors = []
    for u, v in G.edges():
        u_attr = G.nodes[u].get(attr_key, 'Unknown')
        v_attr = G.nodes[v].get(attr_key, 'Unknown')
        if u_attr == v_attr:
            edge_colors.append('green')  # Internal edge
        else:
            edge_colors.append('lightgray')  # External edge
    
    nx.draw_networkx_edges(G, pos, ax=ax, alpha=0.3, width=0.5, edge_color=edge_colors)
    ...
    ax.set_title(f'Attribute-Based Clustering - {period}\n{attr_name}\n'
                f'Cohesion: {cohesion:.1f}% (Internal edges: {internal_edges}, External: {external_edges})',
                fontsize=14, fontweight='bold', pad=15)
    ax.axis('off')
...
def main():
    ...
    print("\n" + "="*80)
    print("ANALYSIS 4: Clustering Trends")
    print("="*80)
    
    # Create clustering trend charts
    for attribute in attributes:
        create_clustering_trend_chart(period_data, attribute)
