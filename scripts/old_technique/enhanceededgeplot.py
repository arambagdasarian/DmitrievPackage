...
class TemporalNetworkAnalyzer:
    ...
    def create_3d_temporal_visualization(self):
        """
        Create enhanced 3D visualization with smaller nodes and connection line controls
        """
        print("\nCreating enhanced 3D temporal visualization with interactive controls...")
        ...
        fig.update_layout(
            title={
                'text': "3D Temporal Network Analysis - Full Window View<br><sub>Smaller nodes • Connection line controls • Node height = Composite centrality</sub>",
                'x': 0.5,
    ...
    def create_individual_period_visualizations(self):
        ...
        for period_name, G in self.networks.items():
            ...
            fig.update_layout(
                title={
                    'text': f"3D Network - {G.graph['display_name']}<br><sub>Smaller nodes • Connection line controls</sub>",
                    'x': 0.5,
...
def main():
    ...
    print("\n=== GENERATING FULL WINDOW VISUALIZATIONS ===")
    
    # Create main 3D temporal visualization
    main_fig = analyzer.create_3d_temporal_visualization()
    main_fig.write_html('temporal_network_3d_fullwindow.html')
    print("Full window 3D visualization saved to: temporal_network_3d_fullwindow.html")
    ...
    for period_name, fig in individual_figs.items():
        filename = f'network_3d_fullwindow_{period_name.lower().replace("-", "_")}.html'
        fig.write_html(filename)
        print(f"Full window {period_name} visualization saved to: {filename}")
    
    # Create network metrics plot
    metrics_fig = analyzer.create_network_metrics_plot()
    metrics_fig.write_html('network_evolution_metrics_fullwindow.html')
    print("Full window metrics plot saved to: network_evolution_metrics_fullwindow.html")
    
    print("\n=== VISUALIZATION COMPLETE ===")
    print("\nKey improvements:")
    print("✓ SMALLER node sizes (4-15px range)")
    print("✓ Full window expansion (autosize=True, width=None, height=None)")
    print("✓ Connection line visibility controls in ALL HTML files")
    print("✓ Enhanced spacing (scale=5.0, ranges=[-10,10])")
    print("✓ Interactive buttons for node/connection toggles")
    print("✓ Minimized margins for maximum plot area")
